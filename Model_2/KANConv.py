...
class KAN_Convolutional_Layer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, update_grid=False):
        print("input shape of kan conv", x.shape)
        # If there are multiple convolutions, apply them all
        if self.n_convs>1:
            out = convolution.multiple_convs_kan_conv2d(x, self.convs,self.kernel_size[0],self.stride,self.dilation,self.padding,self.device)
            print("out shape of kan conv", out.shape)
            return out
        out = self.convs[0].forward(x)
...
class KAN_Convolution(torch.nn.Module):
    def __init__(
        ...
        self.device = device
        self.conv = KAN(
            layers_hidden=[math.prod(kernel_size),1],
            grid_size=grid_size,
            spline_order=spline_order,
            scale_noise=scale_noise,
            scale_base=scale_base,
...
class Mutable_KAN_Conv(ParametrizedModule, KAN_Convolutional_Layer, wraps=KAN_Convolutional_Layer): pass
